...
- [x] Disease — infection spreading between nearby agents; medicine/herbs reduce risk or duration; sick agents drain faster or gather less
...
- [x] Predators — wolves/bears that can reduce agent health when adjacent; low-health or young agents at higher risk
- [ ] Animal populations — reproduction, migration, or extinction logic per species (or per spawn bucket)
- [x] Domestication feedback — tamed animals near buildings periodically provide food or resources to gather

### Society & concepts
- [ ] Era 3+ — philosophy, governance, trade concepts (prereqs from writing/community)
- [x] Trade — agents with "trade" concept exchange items when close (e.g. food for wood, ore for fish)
- [x] Visible buildings from concepts — workshop (stone_tools + weaving), shrine (art + community), etc., when agent rests
...
- [x] Progressive Web App — service worker + manifest; installable, offline-capable
...
- [x] Seasonal migration — agents (or animals) bias movement toward better tiles in winter
- [x] Natural disasters — drought, flood, blight (temporary tile or resource effects; seed + day for reproducibility)
